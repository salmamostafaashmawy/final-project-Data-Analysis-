let upload=document.getElementById('uploadconfirm').addEventListener('click',()=>{
    ...
    Papa.parse(document.getElementById('uploadfile').files[0],
    {
        ...
        var option2=""
        var sum=Object.keys(results.data[0]);
        for(i in sum){
            option1+= '<option value="' +sum[i]+'">'+ sum[i]+"</option>"
        }
        ...
        var option1=""
        var regression=Object.keys(results.data[0]);
        for(i in regression){
            option1+= '<option value="' +regression[i]+'">'+ regression[i]+"</option>"
        }
        document.getElementById('dep').innerHTML+=option;
        document.getElementById('ind').innerHTML+=option;
        document.getElementById('selectreg').addEventListener('click',()=>{
            //
        });
